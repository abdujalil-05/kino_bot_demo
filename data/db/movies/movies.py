...
def get_movie(movie_id):
    with get_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute("SELECT * FROM items WHERE id = %s", (movie_id,))
            movie_item = cursor.fetchall()
            if movie_item:
                if movie_item[0][1]:
                    movie_code = movie_item[0][3]
                    cursor.execute("SELECT * FROM movies WHERE id = %s", (movie_code,))
                    data = cursor.fetchall()
                    data.append({"is_movie": True})
                    return data
                else:
                    episode_code = movie_item[0][4]
                    cursor.execute("SELECT * FROM episodes WHERE id = %s", (episode_code,))
                    data = cursor.fetchall()
                    data.append({"is_movie": False})
                    return data
            return []
...
def delete_movie(movie_code):
    with get_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute("SELECT * FROM items WHERE id = %s", (movie_code,))
            movie_id = cursor.fetchall()
            if movie_id:
                cursor.execute("DELETE FROM movies WHERE id = %s", (movie_id[0][3],))
                cursor.execute("DELETE FROM items WHERE id = %s", (movie_code,))
                conn.commit()

                cursor.execute("SELECT * FROM items WHERE id = %s", (movie_code,))
                data_item = cursor.fetchall()
                cursor.execute("SELECT * FROM movies WHERE id = %s", (movie_id[0][3],))
                data_movie = cursor.fetchall()
